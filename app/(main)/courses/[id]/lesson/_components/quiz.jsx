...
export const Quiz = ({ courseId, isQuizTaken, quizSet }) => {
  const [open, setOpen] = useState(false);
  const totalMark = quizSet?.quizIds?.reduce((acc, cv) => {
    return acc + cv.mark;
  }, 0);
      ...
      <div class="max-w-[270px] bg-white border border-border rounded-md dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-4">
          ...
          <p class="mb-4 font-normal text-gray-500 dark:text-gray-400 text-sm">
            Taking the quiz is optional but it is highly recommanded.
          </p>
          <Button
          ...
          >
            ...
            <span>{isQuizTaken ? "Quiz Taken" : "Participate in a Quiz"}</span>
          </Button>
        </div>
      </div>
      <QuizModal
